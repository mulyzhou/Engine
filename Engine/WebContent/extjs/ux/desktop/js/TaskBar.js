...
Ext.extend(Ext.ux.TaskBar, Ext.util.Observable, {
    init : function(){
		...
		this.msgMenu = new Ext.ux.MsgMenu({
			iconCls: 'user',
			height: 150,
			shadow: true,
			hidden : false,
		...
		this.taskMenu = new Ext.flying.Menu({
			grid : this.creatTaskGrid()
		});
		
		this.taskMenu.on("show",this.msgMenuShow,this);
       	
		this.taskMenu.on("hide",this.msgMenuHide,this);
		
		this.taskBtn = new Ext.Button({
			id:"ux-task-panel-btn",
			text:'(0)',
			iconCls : 'flyingTask',
			style : 'margin:5px 0px 0px 5px;color:#000;',
            menuAlign: 'br-tr',
            menu: this.taskMenu,
            renderTo: 'ux-msg-panel',
            clickEvent:'mousedown'
        ...
        var container = new Ext.ux.TaskBarContainer({
			el: 'ux-taskbar',
			layout: 'border',
			items: [sbBox,this.tbPanel,this.msgPanel]
		});
	...
	creatTaskGrid : function(){
    	...
    	this.taskDs = new Ext.data.Store({// 数据源
    		autoLoad : true,
    		remoteSort : true,
    	...
    	var showDetailsRender = function(val,last,row){
    	...
    	this.taskGrid = new Ext.grid.FlyingGridPanel({// 列表
    		title : '提醒',
